...
float velocity_ref = 50; // 参考速度 (rad/s)
...
/* PID 参数 */
struct PIDController pid_velocity = {.P = 0.003389f, .I = 0.00144f, .D = 0.0f, .output_ramp = 100000, .limit = 24, .error_prev = 0, .output_prev = 0, .integral_prev = 0, .Ts = 1E-4f};

/* 低通滤波 */
struct LowPassFilter filter_velocity = {.alpha = 0.9f, .y_prev = 0.0f};

/* 更新频率 */
int steps_velocity = 1;     // 速度环运行间隔 (当ADC低端电流采样中断周期为1e-4时，速度环运行周期为 steps_velocity * 1e-4)
int count_velocity = 1 - 1; // 保证第一次计数即可执行速度环
    ...
    if (hadc == &hadc1)
    {
        ...
        else
        {
            ...
            if (count_velocity >= steps_velocity)
            {
                ...
                velocity_error = velocity_error / PI * 180;
